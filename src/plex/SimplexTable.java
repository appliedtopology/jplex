...
public final class SimplexTable implements Iterable<Simplex> {
  ...
  private int size = 0;
  private int entry_limit = 0;
  private int mask = 0;
  private Simplex[] table;
  private static final double LOAD_FACTOR = 1.4;
  private int MAX_TABLE_SIZE = (1 << 30);
  private int MAX_ENTRY_LIMIT = (int) Math.floor(MAX_TABLE_SIZE / LOAD_FACTOR);
  ...
  private void grow_table(int newLimit) {
   ...
   */
  public final Simplex get(Simplex s) {
    int hash = s.hashCode();
    int index = hash & mask;
   ...
   *
   */
  public final Simplex put(Simplex s) {
    int hash = s.hashCode();
    int index = hash & mask;
    int reprobe_index = index;
    if (size >= entry_limit) 
  ...
  private static final class SimpleTableIterator implements Iterator<Simplex> {
    ...
    private SimpleTableIterator(SimplexTable tbl) {
      current_index = -1;
      current_count = 0;
      start_size = tbl.size;
      vec = tbl.table;
