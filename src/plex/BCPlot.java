...
public class BCPlot extends JPanel {
  ...
  private boolean plotTypeScatter = false;
  ...
  //
  private static final int MAX_DISPLAYABLE_BARS = MAX_GRID_HEIGHT_PER_PAGE - 1;
  ...
  private Action fileOpenMenuHandler, fileSaveMenuHandler, fileExitMenuHandler;
  ...
  private int heightOfBaseRegionUsedForPlotLegendRulerLabels = 75;
  private int widthOfSideRegionUsedForPlotLegendRulerLabels = 100;

  private int leftMargin = widthOfSideRegionUsedForPlotLegendRulerLabels/2;
  private int rightMargin = widthOfSideRegionUsedForPlotLegendRulerLabels/2;

  private String legendMajorString = "", legendMinorString = null;

  // Set Visualization Region Size
  private int fullVisualizationWidth = 1200;

  // Width of the grid for the plot
  private int gridWidth;

  // Height of the grid for the plot 10 inch at 1500 dpi
  private int gridHeight = MAX_GRID_HEIGHT_PER_PAGE;
  ...
  // barcode plot lower bound
  private double plotLowerBound;

  private Color fillColor, outlineColor;
  private Color gridRulerFontColor, tickUpperBoundColor, 
    gridTickMinorColor, gridTickMajorColor;
  private Color upperBoundFontColor, labelFontColor, 
    legendMinorFontColor, legendMajorFontColor;
  private Color fixedRulerBorderColor, fixedRulerColor, barcodeBarColor;
  private Color fixedRulerFontColor, 
    fixedRulerMinorTickColor, fixedRulerMajorTickColor;
  private Font labelFont, legendMinorFont, legendMajorFont, 
    gridRulerFont, fixedRulerFont, upperBoundFont;
  private FontMetrics upperBoundFontMetrics, 
    legendMinorFontMetrics, legendMajorFontMetrics;
  private FontMetrics fixedRulerFontMetrics, labelFontMetrics, gridRulerFontMetrics;

  // Writable Region
  private int upperMargin, lowerMargin;

  // Barcode Parameters
  private int barcodeBarHeight, barcodeBarSpacing;

  // Special line shape used in barcode plot to indicate infinite barcode bar
  ...
  private Boolean plotBeingSaved = false;
  ...
  // The Following are only used IF IMPLEMENTING FILE LOAD STUFF
   ...
   */
  private void setMajorLegend(String s){
    legendMajorString = s;
  }
  /**
   * Sets the Minor Legend
   */
  private void setMinorLegend(String s){
    legendMinorString = s;
  }
  ...
  // Private constructor.
  private BCPlot(double[][] values, double upperBound, boolean forceScatter) {
  ...
  private static String illegal_interval_string(int i, double a, double b) {
   ...
   *
   */
  private void SPlot(double[][] values, double upperBound) {
    validate(values, upperBound);
   ...
   */
  private void validate(double[][] numbers, double upperBound) {
    if (numbers == null)
   ...
   */
  private class fileSaveMenuHandler extends AbstractAction {
    private static final long serialVersionUID = 1L;
    private fileSaveMenuHandler(String text, ImageIcon icon,
  ...
  /*
   * This handles the File Exit Menu
   */
  private class fileExitMenuHandler extends AbstractAction {
    private static final long serialVersionUID = 1L;
    private fileExitMenuHandler(String text, ImageIcon icon, String desc,
                                Integer mnemonic) {
      super(text, icon);
      putValue(SHORT_DESCRIPTION, desc);
      putValue(MNEMONIC_KEY, mnemonic);
  ...
  private class DrawingPane extends JPanel {
    ...
    private DrawingPane(){
      ...
      outlineColor = new Color(0x9c,0xc6,0xdf);
      ...
      upperBoundFont = new Font("sansserif", Font.PLAIN, 10);
      upperBoundFontColor =  new Color(0xff,0x00,0x00);
      upperBoundFontMetrics = this.getFontMetrics(upperBoundFont);
     ...
     * tick
     */
    private void drawRuler(Graphics g, int position){
      int xOffset = !plotTypeScatter ? 0 : leftMargin;
                              ...
                              double stringRotationPiRadians)
    {
      ...
      int stringWidth1=0, stringWidth2 = 0, x0=0, x1=0, y0=0, y1=0,
        fontAscent1=0, fontAscent2=0, fontHeight1=0, fontHeight2=0;
     ...
     */
    private void saveImage(File file, String format){
    ...
    /*
     * Loads an in image into the background of the barcode plot
     */
    private void loadImage(File file){
      ...
      try {
        ...
        g = bufferedImage.getGraphics();
        g.drawImage(bufferedImage, 0, 0, null);
        thereIsaBackgroundImage = true;
        backgroundImageName = file.getPath();
      } catch (IOException e) {
        bufferedImage =
          new BufferedImage(fullVisualizationWidth, fullVisualizationHeight,
   ...
   */
  private static void plot(String legend, double[][] values, 
