 ...
 */
/**
 * The <code>WitnessStream</code> class implements Witness complex
 * creation.
 *
 * @version $Id$
 */
public final class WitnessStream extends SimplexStream {
  ...
  private final static int m_index(int n, int k, int N) {
    assert((k > 0) && (1 <= n) && (n <= N));
    return ((k-1) * N) + (n-1);
  }

  //
  // The following package-private static members are used by
  // LazyWitnessStream.
  //

  // Compute distance matrix entry for given landmark/data_point.
  static final double distance_ln(int l, int n, int[] landmarks, PointData data) {
    if (landmarks[l] == n)
  ...
  final static int D_index(int l, int n, int L, int N) {
    assert((l > 0) && (l <= L) && (n > 0) && (n <= N));
    return ((l-1) * N) + (n-1);
  }


  final static double min(double x, double y) {
    if (x < y)
      return x;
    else
      return y;
  ...
  final static double max(double x, double y) {
    if (x > y)
      return x;
    else
      return y;
  ...
  final static int max(int x, int y) {
    if (x > y)
      return x;
    else
      return y;
  ...
  //
  // The preceding package-private static methods are used by
  // LazyWitnessStream.
  //
  ...
  private final SimplexStream.Stack 
