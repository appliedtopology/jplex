...
public final class ExplicitStream extends SimplexStream {
  ...
  private double[] persistence_parameters;
  ...
  private HashMap<Simplex,Double> simplex_table;
  ...
  private HashSet<Double> parameters_table;
  ...
  public static class DComplex {
    ...
    public boolean hasNext() {
      if (!parent_is_consistent())
        ...
        return (dimension < parent.maxDimension()-1);
    }
  ...
  private static int[] toInt(double[] in) {
    if (in == null)
      return null;
   ...
   */
  public final void remove(int[] vertices) {
    if (closed)
      open();
   ...
   * @param      vertices The vertex array of the Simplex to remove.
   */
  public final void prune(int[] vertices) {
    if (closed)
      open();
   ...
   * from Matlab.
   */
  public final void remove(double[] vertices) {
   ...
   * from Matlab.
   */
  public final void prune(double[] vertices) {
   ...
   * from Matlab.
   */
  public final void remove(int[][] simplex_array) {
   ...
   * from Matlab.
   */
  public final void prune(int[][] simplex_array) {
    for (int i = 0; i < simplex_array.length; i++) {
   ...
   */
  public final void remove(double[][] simplex_array) {
   ...
   */
  public final void add(int[] vertices, double parameter) {
    Simplex s = Simplex.getSimplex(vertices);
    add(s, parameter);
  }
   ...
   * @param parameter The persistence parameter of the Simplex to add.
   */
  public final void add(double[] vertices, double parameter) {
   ...
   */
  public final void add(int[][] simplex_array, double[] parray) {
   ...
   * @param parray The array of associated persistence parameters.
   */
  public final void add(double[][] simplex_array, double[] parray) {
  ...
  private void ensure_faces(Simplex s, Double P, HashMap<Simplex,Double> pending) {
  ...
  // Realloc() for int[].
  private final int[] ensure_length(int[] v, int index) {
    if (index < v.length)
      return v;
  ...
  private boolean faces_are_consistent(Simplex s, Double P,
  ...
  private static final class ExplicitStreamIterator implements Iterator<Simplex> {
    ...
    private ExplicitStreamIterator(ExplicitStream str) {
      current_dimension = 0;
      current_index = 0;
    ...
    private ExplicitStreamIterator(ExplicitStream str, int dimension) {
  ...
  /**
   * MissingFaceInfo instances record the cofaces (and their associated
   * persistence values) that gave rise to a missing face.
   */
  public static class MissingFaceInfo {
    public final Simplex face;
    public final ArrayList<Simplex> cofaces;
    public final ArrayList<Double> persistence_values;
		
    public MissingFaceInfo(Simplex face) {
      this.face = face;
      cofaces = new ArrayList<Simplex>();
      persistence_values = new ArrayList<Double>();
    }
		
    public String toString() {
      assert(cofaces.size() == persistence_values.size());
      int count = cofaces.size();
      String return_value = String.format("{ Missing %s (%d cofaces):", face.toString(), count);
      for (int i = 0; i < count; i++) {
        return_value += String.format(" (%s, %s)%s",
                                      cofaces.get(i).toString(),
                                      persistence_values.get(i).toString(),
                                      (i < (count-1))?",":"");
      }
      return_value += " }";
      return return_value;
  ...
  /**
   * If the stream is open, iterate over all of the faces, checking to see
   * if it is complete. If you find a missing face, record the cofaces of
   * dimension one greater that should have given rise to that face,
   * together with the persistence values for those cofaces. NOTE: This
   * doesn't return all of the higher dimensional cofaces, as that seems
   * redundant. Naturally, if a simplex is missing, it might be the case
   * that all of its faces are missing, too, but it seems better to return
   * information that is likely to be pertinent, rather than to bury the
   * user - if, for instance, a 5-dimensional face is missing, then
   * probably all of its edges are missing, too, but to have the stream
   * contain those missing edges and all of the cofaces that "should have"
   * given rise to that edge would be a welter of data that is almost
   * certainly irrelevant.
   * @return An iterator of the MissingFaceInfo instances for the missing
   * faces, or null if there are no missing faces, or if the stream is
   * closed.
   */
  public Iterator<MissingFaceInfo> missing_face_info() {
    if (closed)
      return null;
    else {
      HashMap<Simplex, MissingFaceInfo> mfi_table = new
			HashMap<Simplex, MissingFaceInfo>();
      for(Map.Entry<Simplex,Double> E : simplex_table.entrySet()) {
        Simplex s = E.getKey();
        Double P = E.getValue();
        assert(P instanceof Double);
        ...
        while (fi.hasNext()) {
          ...
          Double faceP = simplex_table.get(face);
          if (faceP == null) {
            MissingFaceInfo mfi = mfi_table.get(face);
            if (mfi == null) {
              mfi = new MissingFaceInfo(face);
              mfi_table.put(face, mfi);
            }
            mfi.cofaces.add(s);
            mfi.persistence_values.add(P);
          }
        }
      }
      if (mfi_table.size() == 0)
        ...
        return mfi_table.values().iterator();
    }
  }
}
