...
//  ***************************************************************************
// 
//  Fastest, most compact form of Simplex. Can be used for H2 calculations.
// 
//  $Id$
// 
...
public final class Packed2Simplex extends Simplex {
  ...
  // Simplex.
  private static int VERTEX_BIT_SZ = 32;
  private static final int MAX_2_INDEX = Integer.MAX_VALUE;
  private static final long DIM_0_MASK = ~((long) MAX_2_INDEX);
  ...
  private Packed2Simplex(long bits) {
    this.bits = bits;
  }

  private static final boolean check_2_vertices(int[] v) {
    if (v.length == 0)
      return true;
    if ((v.length == 1) && (v[0] > 0))
      return true;
  ...
  private static final void assert_2_vertices(int[] v) {
  ...
  private final int compareTo(Packed2Simplex s) {
    if (bits > s.bits)
      return 1;
    else if (bits < s.bits)
      return -1;
  ...
  public static final Simplex makeEdge(int v1, int v2) {
    if ((v1 < 1) || (v2 < 1) || (v1 == v2))
      throw new 
        IllegalArgumentException
  ...
  public static final Simplex makePoint(int v) {
    if (v < 1)
      throw new 
        IllegalArgumentException
