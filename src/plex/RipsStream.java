...
public final class RipsStream extends SimplexStream {
  ...
  final static int edge_index(int i, int j, int n) {
    assert((i > 0) && (i < j) && (j <= n));
  ...
  final static int all_edges_present(int[] verts, 
                                     int[] edge_info, int n) {
  ...
  //
  // The previous package-private static members are used by
  // LazyWitnessStream.
  //
  ...
  private final SimplexStream.Stack find_simplices(double max_dist, 
                                                   int max_dimension, 
                                                   PointData data) {
    if (max_dimension < 0)
      return null;
